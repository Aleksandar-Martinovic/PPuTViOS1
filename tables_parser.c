...
ParseErrorCode parsePmtElementaryInfo(const uint8_t* pmtElementaryInfoBuffer, PmtElementaryInfo* pmtElementaryInfo)
{
	...
	higher8Bits = *(pmtElementaryInfoBuffer + 1) & 0x1F;
	lower8Bits = *(pmtElementaryInfoBuffer + 2);
	all16Bits = (higher8Bits << 8) + lower8Bits;

	pmtElementaryInfo->elementaryPid = all16Bits;

	higher8Bits = *(pmtElementaryInfoBuffer + 3) & 0x0F;
	lower8Bits = *(pmtElementaryInfoBuffer + 4);
	all16Bits = (higher8Bits << 8) + lower8Bits;

	pmtElementaryInfo->esInfoLength = all16Bits;
